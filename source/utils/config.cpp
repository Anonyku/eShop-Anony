...
void Config::initialize() {
	FILE *temp = fopen("sdmc:/3ds/Ghost-Eshop/Config.json", "w");
	char tmp[2] = { '{', '}' };
...
*/
Config::Config() {
	if (access("sdmc:/3ds/Ghost-Eshop/Config.json", F_OK) != 0) {
		this->initialize();
	}

	FILE *file = fopen("sdmc:/3ds/Ghost-Eshop/Config.json", "r");
...
void Config::save() {
	if (this->changesMade) {
		FILE *file = fopen("sdmc:/3ds/Ghost-Eshop/Config.json", "w");
